...
import com.example.work.zsproject.view.adapter.NotpassAdapter;
...
public class NotpassFragment extends Fragment implements View.OnClickListener {
    ...
    private XRecyclerView mMopassXrecycleView;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.mopass_layout, container, false);
        initView(view);
        getData();
        return view;
    }
    ...
    private void initView(View view) {
        ...
        mMopassXrecycleView = (XRecyclerView) view.findViewById(R.id.mopass_xrecycleView);

        notpassAdapter = new NotpassAdapter(getContext());
        list = new ArrayList<>();
    }
    ...
    private void getData() {
        ...
        //设置布局
        mMopassXrecycleView.setLayoutManager(new LinearLayoutManager(getContext()));
        //设置适配器
        mMopassXrecycleView.setAdapter(notpassAdapter);

        for (int i = 0; i < 15; i++) {
            list.add("条目" + i);
        }

        notpassAdapter.setList(list);

        mTitleVehicle.setText("车辆（" + list.size() + "）");
        //XRecyclerView的上下拉监听方法
        mMopassXrecycleView.setLoadingListener(new XRecyclerView.LoadingListener() {
            @Override
            //下拉刷新
            public void onRefresh() {
                Toast.makeText(getContext(),"正在完善此功能...",Toast.LENGTH_LONG).show();
                mMopassXrecycleView.refreshComplete();
            }
